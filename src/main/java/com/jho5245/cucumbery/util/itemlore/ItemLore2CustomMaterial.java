  ...
  protected static void setItemLore(@Nullable Player viewer, @NotNull CustomMaterial customMaterial, @NotNull ItemStack itemStack, @NotNull ItemMeta itemMeta, @NotNull List<Component> lore)
  {
    ...
    switch (customMaterial)
    {
      case THE_MUSIC -> {
        ...
        lore.add(ComponentUtil.translate("&6능력 : %s %s", ComponentUtil.translate("&a'그 노래' 재생"), ComponentUtil.translate("rg255,204;&l우클릭")));
      ...
      case STONK -> {
        ...
        lore.add(Component.empty());
        lore.add(ComponentUtil.translate("&6능력 : %s %s", ComponentUtil.translate("&a채광 부스터"), ComponentUtil.translate("rg255,204;&l우클릭")));
        lore.add(ComponentUtil.translate("rg255,204;10초간 채광 속도가 3배로 증가"));
        lore.add(ComponentUtil.translate("&7재사용 대기시간 : %s", ComponentUtil.translate("&a%s초", 120)));
      }
      case PORTABLE_CRAFTING_TABLE -> {
        lore.add(Component.empty());
        lore.add(ComponentUtil.translate("&6능력 : %s %s", ComponentUtil.translate("&a작업대 사용"), ComponentUtil.translate("rg255,204;&l우클릭")));
      }
      case TODWOT_PICKAXE -> {
      ...
      case SANS_BOOTS, SANS_CHESTPLATE, SANS_LEGGINGS, SANS_HELMET -> {
        ...
        lore.add(ComponentUtil.translate("rg255,204;스켈레톤에게 어그로가 끌리지 않음"));
      }
      case FLINT_SHOVEL -> {
        lore.add(Component.empty());
