  ...
  public static void sendInfo(CommandSender sender, ItemStack item, boolean shortend)
  {
    try
    {
      ...
      if (itemMeta instanceof Repairable)
      {
        ...
        {
          MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "&r모루 사용 횟수 : &e" + ItemStackUtil.getAnvilUsedTime(itemMeta) + "번");
        }
        else
        {
      ...
      switch (type)
      {
        ...
        case TIPPED_ARROW:
          ...
          {
            MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "&c커스텀 포션 효과 없음 &r(기본값 : &e" + item + "&r)");
            ...
            if (potionMeta.getBasePotionData().getType().getEffectType() != null)
            {
              ...
              MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "포션 효과 : &e" + potionEffectType + "&r (&e내부 이름 : &e" + potionEffectType.getName() + "&r)");
              MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "포션 최대 레벨 : &e" + maxLevel);
              if (isExtended)
              {
          ...
          {
            MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "커스텀 포션 효과 개수 : &e" + potionMeta.getCustomEffects().size() + "개");
            ...
            if (shortend && potionMeta.getCustomEffects().size() > 5 && sender instanceof Player)
            {
              ...
              for (int i = 0; i < potionMeta.getCustomEffects().size(); i++)
              {
                ...
                msg.add(ComponentUtil.create("[" + (i + 1) + "]",
                        "&r효과 이름 : &e" + (PotionEffectType) + "\n&r농도 레벨 : &e" + amplifier + "단계\n&r지속 시간 : &e" + Method.timeFormatMilli(duration * 50L)));
              }
            ...
            else
            {
              ...
              for (PotionEffect effect : potionMeta.getCustomEffects())
              {
                ...
                int duration = effect.getDuration();
                PotionEffectType PotionEffectType = effect.getType();
                effects.append("&r효과 이름 : &e").append(PotionEffectType).append("&r, 농도 레벨 : &e").append(amplifier).append("단계&r, 지속 시간 : &e").append(
                        Method.timeFormatMilli(duration * 50L)).append("\n");
              }
        ...
        case FIREWORK_ROCKET:
          ...
          MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "폭죽 비행 시간 : &e" + fireworkMeta.getPower());
          ...
          {
            MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "폭죽 효과 개수 : &e" + fireworkMeta.getEffectsSize() + "개");
            if (sender instanceof Player)
            {
              ...
              for (int i = 0; i < fireworkMeta.getEffectsSize(); i++)
              {
                ...
                StringBuilder fireworkMsg = new StringBuilder("&r폭죽 모양 : &e" + (fireworkEffect.getType()) + "\n");
                for (int j = 0; j < fireworkEffect.getColors().size(); j++)
                {
                  ...
                  fireworkMsg.append("&e").append(j + 1).append("번째&r 폭죽 색상 : &e").append(color).append("&r(&c빨강 ").append(red).append("&r, &a초록 ").append(green)
                          .append("&r, &9파랑 ").append(blue).append("&r)\n");
                }
                for (int j = 0; j < fireworkEffect.getFadeColors().size(); j++)
                {
                  ...
                  int green = color.getGreen();
                  int blue = color.getBlue();
                  fireworkMsg.append("&e").append(j + 1).append("번째&r 사라지는 효과 색상 : &e").append(color).append("&r(&c빨강 ").append(red).append("&r, &a초록 ").append(green)
                          .append("&r, &9파랑 ").append(blue).append("&r)\n");
                }
                if (fireworkEffect.hasTrail())
                {
                  fireworkMsg.append("&e잔상 효과&r\n");
                }
                if (fireworkEffect.hasFlicker())
                {
                  fireworkMsg.append("&e반짝이는 효과&r\n");
                }
            ...
            else
            {
              ...
              {
                FireworkEffect fireworkEffect = fireworkMeta.getEffects().get(i);
                StringBuilder fireworkMsg = new StringBuilder(Prefix.INFO_ITEMSTORAGE + "폭죽 모양 : &e" + fireworkEffect.getType() + "\n");
                for (int j = 0; j < fireworkEffect.getColors().size(); j++)
                {
                  Color color = fireworkEffect.getColors().get(j);
                  int red = color.getRed();
                  int green = color.getGreen();
                  int blue = color.getBlue();
                  fireworkMsg.append("&e").append(j + 1).append("번째&r 폭죽 색상 : &e").append(color).append("&r(&c빨강 ").append(red).append("&r, &a초록 ").append(green).append(
                          "&r, &9파랑 ").append(blue).append("&r), ");
                }
                for (int j = 0; j < fireworkEffect.getFadeColors().size(); j++)
                {
                  Color color = fireworkEffect.getFadeColors().get(j);
                  ...
                  int green = color.getGreen();
                  int blue = color.getBlue();
                  fireworkMsg.append("&e").append(j + 1).append("번째&r 사라지는 효과 색상 : &e").append(color).append("&r(&c빨강 ").append(red).append("&r, &a초록 ").append(green).append(
                          "&r, &9파랑 ").append(blue).append("&r), ");
                }
                if (fireworkEffect.hasTrail())
                {
        ...
        case FIREWORK_STAR:
          ...
          else
          {
            ...
            StringBuilder msg = new StringBuilder(Prefix.INFO_ITEMSTORAGE + "폭죽 모양 : &e" + fireworkEffect.getType() + "&r, 폭죽 색상 &e");
            for (int j = 0; j < fireworkEffect.getColors().size(); j++)
            {
              Color color = fireworkEffect.getColors().get(j);
              int red = color.getRed();
              int green = color.getGreen();
              int blue = color.getBlue();
              msg.append("폭죽 색상 : &e").append(color).append("&r(&c빨강 ").append(red).append("&r, &a초록 ").append(green).append("&r, &9파랑 ").append(blue).append("&r)");
            }
            for (int j = 0; j < fireworkEffect.getFadeColors().size(); j++)
            {
              Color color = fireworkEffect.getFadeColors().get(j);
              ...
              int green = color.getGreen();
              int blue = color.getBlue();
              msg.append(", 사라지는 효과 색상 : &e").append(color).append("&r(&c빨강 ").append(red).append("&r, &a초록 ").append(green).append("&r, &9파랑 ").append(blue).append("&r)");
            }
            if (fireworkEffect.hasTrail())
            {
              msg.append(", &e잔상 효과&r");
        ...
        case TROPICAL_FISH_BUCKET:
          ...
          if (bucketMeta.hasVariant())
          {
            ...
            MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "물고기 종 : &e" + item);
          }
          else
          {
        ...
        case SUSPICIOUS_STEW:
          ...
          else
          {
            for (PotionEffect effect : stewMeta.getCustomEffects())
            {
              ...
              int duration = effect.getDuration();
              int amplifier = effect.getAmplifier();
              MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "&e" + count + "번&r째 효과 - 이름 : &e" + id + "&r, 지속 시간 : &e" + Method.timeFormatMilli(duration * 50L) + "&r, 증폭 : &e" + amplifier);
            }
          }
          // try
          // {
          ...
          // msg.add(ComponentUtil.create("[" + (i + 1) + "]", "포션 효과 : &e" + effectType + "&r, 지속 시간 : &e" + Constant.Sosu2.format(duration / 20D) + "&e초"));
          // }
          // Component[] arr = new Component[msg.size()];
          // MessageUtil.sendMessage(sender, msg.toArray(arr));
          // }
          ...
          // int id = Integer.parseInt(effect.split("\\,")[1]);
          // String effectType = PotionName.getPotionEffectName(id);
          // MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "포션 효과 : &e" + effectType + "&r, 지속 시간 : &e" + Constant.Sosu2.format(duration / 20D) + "&e초");
          // }
          // }
          // }
          //
        ...
        case FILLED_MAP:
          ...
          if (mapMeta.hasMapView())
          {
            ...
            MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "&e지도 ID : " + id + "&r");
            MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "&e축척 : " + scaleString + "&r");
            MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "&e월드 : " + worldName + "&r");
            MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "&e지도 중심 좌표 : x=" + centerX + ", z=" + centerZ + "&r");
          }
          else
          {
        ...
        case LEATHER_LEGGINGS:
          ...
          MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "가죽 갑옷 색상 : &e" + (color) + "&r(&c빨강 " + red + "&r, &a초록 " + green + "&r, &9파랑 " + blue + "&r)");
          break;
        case ACACIA_SIGN:
        case BIRCH_SIGN:
        case DARK_OAK_SIGN:
        ...
        case SPRUCE_SIGN:
          ...
          if (lineSize > 0)
          {
            ...
            for (int i = 0; i < sign.getLines().length; i++)
            {
              ...
              if (line.length() > 0)
              {
                MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "&6" + (i + 1) + "&6번째 줄&e : " + line + "&r");
              }
            }
            ...
            for (int i = 0; i < sign.getLines().length; i++)
            {
              ...
              if (line.length() > 0)
              {
                MessageUtil.sendMessage(sender, Prefix.INFO_ITEMSTORAGE, "&6" + (i + 1) + "&6번째 줄&e : " + line.replace("§", "&") + "&r");
              }
            }
          }
        default:
