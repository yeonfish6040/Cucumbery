...
public class Method extends SoundPlay
{
  ...
  public static void updateItem(@NotNull Item itemEntity, int mergeAmount)
  {
    ...
    if (Cucumbery.config.getBoolean("glow-dropped-items") || NBTAPI.arrayContainsValue(extraTags, Constant.ExtraTag.GLOWING))
    {
   ...
   *
   * @param entity    값을 가져올 개체
   * @param attribute 값을 가져올 attribute
   * @return 보기 좋은 문자열
   */
  public static String attributeString(Attributable entity, Attribute attribute)
  {
    ...
    if (differ != 0d)
    {
      attrString += " &r(&e" + Constant.Sosu4.format(attrBaseValue) + "&e " + differString + "&r)";
    }
  ...
  {
    int year = calendar.get(Calendar.YEAR), month = calendar.get(Calendar.MONTH) + 1, date = calendar.get(Calendar.DATE), hour = calendar.get(Calendar.HOUR_OF_DAY), minute = calendar.get(
            Calendar.MINUTE), second = calendar.get(Calendar.SECOND), ms = calendar.get(Calendar.MILLISECOND);
    if (all)
    {
      String time = year +
              "년 " +
              ...
              (((second >= 10) ? "" : "0") + second) +
              "초 " +
              (displayMs ? ((((ms < 100) ? "0" : "") + ((ms < 10) ? "0" : "") + ms) + "ms") : "");
      if (time.endsWith(" "))
      {
   ...
   * @param placeholder 플레이스 홀더 사용 여부
   * @param calculate   수식 계산 여부
   */
  public static void performCommand(@NotNull CommandSender sender, @NotNull String command, boolean placeholder, boolean calculate, @Nullable Object extraParams)
  {
    if (placeholder)
    {
  ...
  public static List<String> tabCompleterDoubleRadius(
          String[] args, double from, boolean excludeFrom, double to, boolean excludeTo, String key, String... additionalArgs)
  {
    String tabArg = args[args.length - 1];
  ...
  public static boolean useItem(Player player, ItemStack item, EquipmentSlot slot)
  {
    ...
    // 명령 블록 빠른 실행
    if (UserData.USE_QUICK_COMMAND_BLOCK.getBoolean(player.getUniqueId()) &&
            !player.isSneaking() &&
            player.isOp() &&
            (item.getType() == Material.COMMAND_BLOCK || item.getType() == Material.REPEATING_COMMAND_BLOCK || item.getType() == Material.CHAIN_COMMAND_BLOCK))
    {
