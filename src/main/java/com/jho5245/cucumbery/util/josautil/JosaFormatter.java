...
public class JosaFormatter
{
  ...
  private static final List<Pair<String, String>> josaPairs = Arrays.asList(
          new Pair<>("은", "는"),
          new Pair<>("이", "가"),
          new Pair<>("을", "를"),
          new Pair<>("과", "와"),
  ...
  private static final Pattern simpleFormatPattern = Pattern.compile(simpleFormatRegex);
      ...
      if (groupCount == 5)
      {
        ...
        else
        {
          ...
          if (indexString != null && indexString.length() > 0)
          {
            ...
            { // previous format specifier index
              index = lastArgIndex;
            }
            else if (indexString.endsWith("$"))
            { // argument position indexing
              try
              {
