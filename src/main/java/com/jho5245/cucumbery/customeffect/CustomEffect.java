    ...
    Component description = switch (effectType)
            {
              case CURSE_OF_MUSHROOM ->
                      ComponentUtil.translate("%s 확률로 5초마다 인벤토리에 버섯이 들어옵니다.", "&e" + ((amplifier + 1) / 10d) + "%");
              case FEATHER_FALLING ->
                      ComponentUtil.translate("낙하 피해를 받기 위한 최소 높이가 %sm 증가하고,","&e" + ((amplifier + 1) * 5))
                      .append(Component.text("\n"))
                      .append(ComponentUtil.translate("낙하 피해량이 %s 감소합니다.","&e" + ((amplifier + 1) * 8) + "%"));
              case BLESS_OF_SANS ->
                      ComponentUtil.translate("근거리 공격 피해량이 %s 증가합니다.", "&e" + ((amplifier + 1) * 10) + "%");
              case SHARPNESS ->
                      ComponentUtil.translate("근거리 공격 피해량이 %s 증가합니다.", "&e" + (amplifier + 1.5));
              case SMITE ->
                      ComponentUtil.translate("언데드 개체에게 주는 근거리 공격 피해량이 %s 증가합니다.", "&e" + Constant.Sosu1.format((amplifier + 1) * 2.5));
