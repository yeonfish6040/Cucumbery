 ...
 * @deprecated see {@link CustomEffectType}
 */
@Deprecated
public enum EffectType implements Translatable, EnumHideable
{
  /**
   * 어색함
   */
  ...
  COOLDOWN_ERROR_WARN_SOUND(true, false, false),
  ;
  ...
  EffectType(int maxAmplifier)
  {
    this(maxAmplifier, false);
  }

  EffectType(int maxAmplifier, boolean isNegative)
  {
    this(maxAmplifier, false, true, isNegative);
  }

  EffectType(boolean isKeepOnDeath, boolean isKeepOnQuit, boolean isNegative)
  {
    this(0, isKeepOnDeath, isKeepOnQuit, isNegative);
  }

  EffectType(int maxAmplifier, boolean isKeepOnDeath, boolean isKeepOnQuit, boolean isNegative)
  {
    ...
    return switch (this)
            {
              ...
              case COOLDOWN_ERROR_WARN_SOUND -> "경고 오류 소리 쿨타임";
    ...
    return switch (this)
            {
              ...
              case RESURRECTION_COOLDOWN -> ComponentUtil.translate("%s 버프를 받을 수 없는 상태입니다", EffectType.RESURRECTION);
              ...
              case COMBAT_MODE_MELEE -> ComponentUtil.translate("근거리 전투에 대한 집중력이 비약적으로 상승하여")
                      ...
                      .append(Component.text("\n"))
                      .append(ComponentUtil.translate("이 효과는 %s 효과와 동시에 적용되지 않습니다", ComponentUtil.translate("&a" + EffectType.COMBAT_MODE_RANGED.translationKey())));
              ...
              case PVP_MODE -> ComponentUtil.translate("%s 효과가 있어야 PvP를 할 수 있습니다", ComponentUtil.translate("&a" + EffectType.PVP_MODE_ENABLED.translationKey()));
    ...
    return switch (this)
            {
              ...
              case GLIDING -> 3;
              case VAR_PODAGRA_ACTIVATED, COOLDOWN_ERROR_WARN_SOUND -> 5;
  ...
  public ItemStack getIcon()
  {
    ...
    if (itemStack.getType() == Material.STONE && this != EffectType.MUNDANE)
    {
      return null;
    }
   ...
   */
  @NotNull
  public List<EffectType> getConflictEffects()
  {
    return switch (this)
            {
              case SHARPNESS -> Arrays.asList(EffectType.SMITE, EffectType.BANE_OF_ARTHROPODS);
              case SMITE -> Arrays.asList(EffectType.SHARPNESS, EffectType.BANE_OF_ARTHROPODS);
              case BANE_OF_ARTHROPODS -> Arrays.asList(EffectType.SHARPNESS, EffectType.SMITE);
              case INSIDER -> Collections.singletonList(EffectType.OUTSIDER);
              case OUTSIDER -> Collections.singletonList(EffectType.INSIDER);
              case HEROS_ECHO -> Collections.singletonList(EffectType.HEROS_ECHO_OTHERS);
              case HEROS_ECHO_OTHERS -> Collections.singletonList(EffectType.HEROS_ECHO);
              case COMBAT_MODE_MELEE -> Collections.singletonList(EffectType.COMBAT_MODE_RANGED);
              case COMBAT_MODE_RANGED -> Collections.singletonList(EffectType.COMBAT_MODE_MELEE);
    ...
    return switch (this)
            {
              case TROLL_INVENTORY_PROPERTY_MIN, VAR_PODAGRA_ACTIVATED, VAR_DETOXICATE_ACTIVATED, BREAD_KIMOCHI_SECONDARY_EFFECT, DISAPPEAR, DAMAGE_INDICATOR, NO_CUCUMBERY_ITEM_USAGE_ATTACK,
                      GLIDING, NOTIFY_NO_TRADE_ITEM_DROP, CUSTOM_DEATH_MESSAGE, COOLDOWN_ERROR_WARN_SOUND-> true;
              default -> false;
            };
  }
  ...
  /**
   * @return {@link EffectType#BUFF_FREEZE} 효과로 인해 사망해도 효과가 사라지지 않으면 true, 이외에는 false
   */
  ...
  public Component getPropertyDescription()
  {
    ...
    if (!buffFreezable)
    {
      description = description.append(Component.text("\n")).append(ComponentUtil.translate("&e%s의 영향을 받지 않습니다", Component.translatable(EffectType.BUFF_FREEZE.translationKey())));
    }
    if (isRealDuration)
    {
      description = description.append(Component.text("\n")).append(
    ...
    if (isToggle())
    {
      description = description.append(Component.text("\n"));
      description = description.append(Component.text("\n"));
      description = description.append(ComponentUtil.translate("&7효과의 지속 시간이 무제한이고 효과가 있을 때"));
      description = description.append(Component.text("\n"));
      description = description.append(ComponentUtil.translate("&7효과를 다시 적용받으면 사라지는 %s", "translate:&6온오프 효과"));
    }
    return description;
  }
}
