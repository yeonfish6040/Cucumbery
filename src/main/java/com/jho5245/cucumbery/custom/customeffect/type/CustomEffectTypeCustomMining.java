...
public class CustomEffectTypeCustomMining extends CustomEffectType
{
  public static final CustomEffectType
          ...
          CUSTOM_MINING_SPEED_MODE_2 = new CustomEffectType("custom_mining_speed_mode_2", "채광 모드 2", builder().defaultDisplayType(DisplayType.NONE).defaultDuration(2).keepOnDeath().skipEvent()),
          CUSTOM_MINING_SPEED_MODE_2_NO_RESTORE = new CustomEffectType("custom_mining_speed_mode_2_no_restore", "채광 모드 2 (복구 없음)", builder().defaultDisplayType(DisplayType.NONE).defaultDuration(2).keepOnDeath().skipEvent()),
  ...
  CUSTOM_MINING_SPEED_MODE_2_NO_RESTORE_IGNORE_INSTA_BLOCKS = new CustomEffectType("custom_mining_speed_mode_2_no_restore_ignore_insta_blocks", "채광 모드 2 (복구 없음, 즉시 파괴 블록 무시)",
          builder().defaultDisplayType(DisplayType.NONE).defaultDuration(2).keepOnDeath().description("%s 효과가 있는 상태에서는 즉시 파괴되는 블록을 부술 땐 채광 모드를 무시합니다", CUSTOM_MINING_SPEED_MODE_2_NO_RESTORE).skipEvent()),
