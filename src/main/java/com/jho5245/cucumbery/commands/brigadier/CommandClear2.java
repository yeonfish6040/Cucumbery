...
public class CommandClear2 extends CommandBase
{
  ...
  private void clear(@NotNull NativeProxyCommandSender sender, @NotNull Collection<Player> players, @NotNull Predicate<ItemStack> predicate, int max)
  {
    ...
    if (total == 0)
    {
      MessageUtil.sendError(sender, ComponentUtil.createTranslate("%s에게 제거할 수 있는 아이템이 없습니다.", players));
    }
    else
    {
      MessageUtil.info(sender, ComponentUtil.createTranslate("%s에게서 아이템 %s개를 제거하였습니다.", players, total));
      MessageUtil.sendAdminMessage(sender, new ArrayList<>(players), ComponentUtil.createTranslate("[%s: %s에게서 아이템 %s개를 제거하였습니다.]",  sender, players, total));
    }
  }
