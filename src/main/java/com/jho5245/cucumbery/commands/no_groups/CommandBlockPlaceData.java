...
public class CommandBlockPlaceData implements CucumberyCommandExecutor
{
            ...
            if (length == 3)
            {
              ...
              if (!itemStack.getType().isAir())
              {
                ...
                if (!overrideWarning.contains(uuid))
                {
                  ...
                  Bukkit.getScheduler().runTaskLater(Cucumbery.getPlugin(), () -> overrideWarning.remove(uuid), 60L);
                  return true;
                }
  ...
  {
    return CucumberyCommandExecutor.legacy(completion(sender, command, label, args, CommandArgumentUtil.senderLocation(sender)));
  }
}
