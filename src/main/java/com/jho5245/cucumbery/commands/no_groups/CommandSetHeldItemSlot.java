...
public class CommandSetHeldItemSlot implements CommandExecutor, TabCompleter
{
	public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, @NotNull String[] args)
	{
		...
		else if (args.length == 1)
		{
			...
			MessageUtil.info(player, "단축바 슬롯을 rg255,204;"+slot+"번&r으로 변경하였습니다");
		}
		else if (args.length <= 3)
		{
			...
			if (!hideMessage)
			{
				...
				{
					MessageUtil.info(target, sender, "이 당신의 단축바 슬롯을 rg255,204;"+slot+"번&r으로 변경하였습니다");
				}
				MessageUtil.info(sender, target, "의 단축바 슬롯을 rg255,204;"+slot+"번&r으로 변경하였습니다");
			}
		}
		else
		{
