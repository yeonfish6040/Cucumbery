...
{
  public static final int CONFIG_VERSION = 41, DEATH_MESSAGES_CONFIG_VERSION = 12, LANG_CONFIG_VERSION = 6;
  ...
  public void onDisable()
  {
    ...
    if (Cucumbery.config.getBoolean("console-messages.plugin"))
    {
      MessageUtil.consoleSendMessage("&5[&d비활성화&5] rg255,204;" + pluginDescriptionFile.getName() + "&r version : rg255,204;" + pluginDescriptionFile.getVersion() + "&r 플러그인이 비활성화 되었습니다");
    }
  }
    ...
    if (config.getBoolean("console-messages.outdated-config"))
    {
      ...
      if (currentConfigVersion != CONFIG_VERSION)
      {
        MessageUtil.consoleSendMessage(Prefix.INFO_WARN, "%s 파일의 버전이 최신 버전과 일치하지 않습니다! 현재 버전 : %s, 최신 버전 : %s",
                "rg255,204;config.yml", currentConfigVersion, CONFIG_VERSION);
      ...
      {
        MessageUtil.consoleSendMessage(Prefix.INFO_WARN, "%s 파일의 버전이 최신 버전과 일치하지 않습니다! 현재 버전 : %s, 최신 버전 : %s",
                "rg255,204;DeathMessages.yml", currentDeathMessagesVersion, DEATH_MESSAGES_CONFIG_VERSION);
        MessageUtil.consoleSendMessage(Prefix.INFO, "%1$s 파일을 삭제하고 플러그인을 리로드하여 %1$s 파일을 재생성하시거나 플러그인 파일에 있는 %1$s에서 직접 값을 붙여넣어 주세요.", "rg255,204;DeathMessages.yml");
      }
      int currentLangVersion = Variable.lang.getInt("config-version");
      if (currentLangVersion != LANG_CONFIG_VERSION)
      {
        MessageUtil.consoleSendMessage(Prefix.INFO_WARN, "%s 파일의 버전이 최신 버전과 일치하지 않습니다! 현재 버전 : %s, 최신 버전 : %s",
                "rg255,204;lang.yml", currentLangVersion, LANG_CONFIG_VERSION);
...
//        brigadierService.submit(this::registerBrigadierCommands);
  ...
  private void checkUsingAddons()
  {
    ...
    Cucumbery.using_NoteBlockAPI = Cucumbery.config.getBoolean("use-hook-plugins.NoteBlockAPI") && this.pluginManager.getPlugin("NoteBlockAPI") instanceof NoteBlockAPI;
    ...
    Cucumbery.using_PlaceHolderAPI = Cucumbery.config.getBoolean("use-hook-plugins.PlaceHolderAPI") && this.pluginManager.getPlugin("PlaceHolderAPI") instanceof PlaceholderAPIPlugin;
